describe('Dashboard with Transloadit', () => {
  ...
  it('should upload cat image successfully', () => {
    ...
    cy.get('.ImgEditTests-StatusBar-actionBtn--upload').click()
    cy.wait(['@assemblies', '@resumable']).then(() => {
      cy.get('.ImgEditTests-StatusBar-statusPrimary').should('contain', 'Complete')
    })
  })
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ ImgEditTests }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(ImgEditTests.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        ImgEditTests.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(ImgEditTests.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ ImgEditTests }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImgEditTests.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ ImgEditTests }) => {
      ...
      const { files } = ImgEditTests.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImgEditTests.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ ImgEditTests }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(ImgEditTests.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it.skip('should create assembly if there is still one file to upload', () => {
    ...
    cy.window().then(({ ImgEditTests }) => {
      ...
      ImgEditTests.removeFile(fileID)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ ImgEditTests }) => {
      const { files } = ImgEditTests.getState()
      ImgEditTests.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        ImgEditTestsserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'ImgEditTests-core:3.2.0,ImgEditTests-transloadit:3.1.3,ImgEditTests-tus:3.1.0,ImgEditTests-dropbox:3.1.1,ImgEditTests-box:2.1.1,ImgEditTests-facebook:3.1.1,ImgEditTests-google-drive:3.1.1,ImgEditTests-instagram:3.1.1,ImgEditTests-onedrive:3.1.1,ImgEditTests-zoom:2.1.1,ImgEditTests-url:3.3.1',
    ...
    cy.get('.ImgEditTests-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.ImgEditTests-StatusBar-actions > .ImgEditTests-c-btn').click()
    cy.wait([
