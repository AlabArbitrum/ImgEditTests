...
describe('Dashboard with XHR', () => {
  ...
  it('should return correct file name with URL plugin from remote image with Content-Disposition', () => {
    ...
    cy.get('.ImgEditTests-Url-input').type(
      'http://localhost:4678/file-with-content-disposition',
    )
    interceptCompanionUrlMetaRequest()
    cy.get('.ImgEditTests-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.ImgEditTests-Dashboard-Item-name').should('contain', fileName)
      cy.get('.ImgEditTests-Dashboard-Item-status').should('contain', '84 KB')
    })
  })

  it('should return correct file name with URL plugin from remote image without Content-Disposition', () => {
    cy.get('[data-cy="Url"]').click()
    cy.get('.ImgEditTests-Url-input').type('http://localhost:4678/file-no-headers')
    interceptCompanionUrlMetaRequest()
    cy.get('.ImgEditTests-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.ImgEditTests-Dashboard-Item-name').should('contain', 'file-no')
      cy.get('.ImgEditTests-Dashboard-Item-status').should('contain', '0')
    })
  })
  ...
  it('should return correct file name even when Companion doesnt supply it', () => {
    ...
    cy.get('.ImgEditTests-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.ImgEditTests-Dashboard-Item-name').should('contain', 'file-with')
      cy.get('.ImgEditTests-Dashboard-Item-status').should('contain', '123 B')
    })
  })
